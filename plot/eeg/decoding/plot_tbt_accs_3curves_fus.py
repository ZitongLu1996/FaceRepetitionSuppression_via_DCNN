...
for sub in range(nsubs):
    subaccs = np.array(h5py.File("../../../eeg/decoding_acc/un-sn_sub" + str(sub + 1) + ".h5", "r")["acc"])
    print(subaccs.shape)
    accs[sub] = subaccs
...
for t in range(100):
    if t >= 25:
        if ps[t] == 1:
            plt.plot(t * 0.02 + 0.01 - 0.5, 0.79, "s", color="deeppink", alpha=0.9, markersize=3)
            xi = [t * 0.02 - 0.5, t * 0.02 + 0.02 - 0.5]
x = np.arange(-0.5 + 0.01, 1.5 + 0.01, 0.02)
plt.fill_between(x, avg + err, avg - err, facecolor="deeppink", alpha=0.9, label="New")

accs = np.zeros([nsubs, 100, 100])
for sub in range(nsubs):
    subaccs = np.array(h5py.File("../../../eeg/decoding_acc/ue-se_sub" + str(sub + 1) + ".h5", "r")["acc"])
    print(subaccs.shape)
    accs[sub] = subaccs
...
for t in range(100):
    if t >= 25:
        if ps[t] == 1:
            plt.plot(t * 0.02 + 0.01 - 0.5, 0.782, "s", color="deeppink", alpha=0.6, markersize=3)
            xi = [t * 0.02 - 0.5, t * 0.02 + 0.02 - 0.5]
x = np.arange(-0.5 + 0.01, 1.5 + 0.01, 0.02)
plt.fill_between(x, avg + err, avg - err, facecolor="deeppink", alpha=0.6, label="Immediate")

accs = np.zeros([nsubs, 100, 100])
for sub in range(nsubs):
    subaccs = np.array(h5py.File("../../../eeg/decoding_acc/ul-sl_sub" + str(sub + 1) + ".h5", "r")["acc"])
    print(subaccs.shape)
    accs[sub] = subaccs
...
for t in range(100):
    if t >= 25:
        if ps[t] == 1:
            plt.plot(t * 0.02 + 0.01 - 0.5, 0.774, "s", color="deeppink", alpha=0.3, markersize=3)
            xi = [t * 0.02 - 0.5, t * 0.02 + 0.02 - 0.5]
x = np.arange(-0.5 + 0.01, 1.5 + 0.01, 0.02)
plt.fill_between(x, avg + err, avg - err, facecolor="deeppink", alpha=0.3, label="Delay")
...
ps3 = clusterbased_permutation_1d_1samp_1sided(rlts3-rlts2, level=0, p_threshold=0.01, iter=1000)
for t in range(100):
    if t >= 25:
        if ps1[t] == 1:
