...
for i in range(8):
    ...
    for j in range(150):
        ...
        for k in range(n1):
            if fj[k] == 0:
                n0 = n0 + 1
        fj[index[n0:int(n0 + (n1 - n0) * 0.49)]] = 0
        f2[j] = pca.transform(np.reshape(fj, [1, n1]))[0]
        fj = np.array(features[j].copy())
        fj[index[n0:int(n0 + (n1 - n0) * 0.01)]] = 0
        f3[j] = pca.transform(np.reshape(fj, [1, n1]))[0]
    for j in range(150):
        j = j + 300
        fj = np.array(features[j].copy())
        ...
        for k in range(n1):
            if fj[k] == 0:
                n0 = n0 + 1
        fj[index[n0:int(n0 + (n1 - n0) * 0.48)]] = 0
        f2[j] = pca.transform(np.reshape(fj, [1, n1]))[0]
        fj = np.array(features[j].copy())
        fj[index[n0:int(n0 + (n1 - n0) * 0.01)]] = 0
        f3[j] = pca.transform(np.reshape(fj, [1, n1]))[0]
    ...
    rdm = bhvRDM(f)
    f = h5py.File("rdms_random_sharpening_3faces/ly" + str((i+1)*2) + ".h5", "w")
