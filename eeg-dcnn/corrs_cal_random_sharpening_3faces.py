...
for ly in range(8):
    vggrdm1350 = np.array(h5py.File("../dcnn/rdms_random_sharpening_3faces/ly"+str((ly+1)*2)+".h5", "r")["rdm"])
    vggrdm = np.zeros([9, 9])
    for i in range(9):
        for j in range(9):
    ...
    for sub in range(18):
        ...
        f = h5py.File("corrs/random_sharpening_3faces/ly" + str((ly+1)*2) + "_sub" + str(sub+1) +".h5", "w")
